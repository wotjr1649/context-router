...
// loadHit: chunkID 1건을 Hit으로 채운다. Source=uri의 base name, Snippet=본문 앞
// 500B는 둘 다 임시 구현이다 — 7b가 project-relative 표시와 매치 중심 창으로 교체한다.
func loadHit(ctx context.Context, db *sql.DB, chunkID int64, score float64) (Hit, error) {
	var h Hit
	var text, redaction, uri, kind string
	err := db.QueryRowContext(ctx, hitQuery, chunkID).
		Scan(&h.ArtifactID, &h.LineStart, &h.LineEnd, &text, &redaction, &uri, &kind)
	if err != nil {
		return Hit{}, fmt.Errorf("search: load chunk %d: %w", chunkID, err)
	}
	h.ChunkID = chunkID
	h.Score = score
	h.Source = path.Base(uri) // 7b: project-relative 표시로 교체
	if len(text) > 500 {
		text = text[:500]
	}
	h.Snippet = text // 7b: 매치 중심 창으로 교체
...
func Query(ctx context.Context, st *store.Store, queries []string, limit, budgetBytes int) ([]QueryResult, error) {
	...
	for _, q := range queries {
		...
		for _, sc := range top {
			h, err := loadHit(ctx, db, sc.id, sc.score)
			if err != nil {
				return nil, err
			}
			hits = append(hits, h)
		}
		results = append(results, QueryResult{Query: q, Hits: hits})
	}
	return results, nil
}
