...
// 블록 밖 canonical 헤더. anomaly: 마커 무결성 이상 또는 키-경계 충돌(→ [16] warning ⑤).
// 우선순위는 installCodexConfigBlock과 동일하다 — canonical 헤더 라인은 스스로 mcp_servers·ctr]
// 신호를 겸해 conflict도 켜므로(예: "[mcp_servers.ctr]"), install처럼 hasHeader를 conflict보다
// 먼저 판정해야 헤더 실존을 존재(mcpExistingHeader)로, 헤더 없는 충돌만 이상(mcpConflict)으로 본다.
func probeCodexMCPBlock(existing []byte) (present bool, anomaly bool) {
	...
	if class == classReplace || hasHeader {
		return true, false
	}
	if conflict {
		return false, true
