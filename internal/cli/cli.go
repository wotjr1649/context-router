...
//
// **중복 발화는 조건형이고 지시가 그 조건에 걸린다(리뷰 I2).** heldGroupHint는 단정형인데 그쪽은
// 마커 자체가 우리 옛 설치기의 산물임을 증명해 "플러그인이 따로 있다"가 함의된다. 여기는 그 증명이
// 없고, 이 절이 읽는 것은 settings.json뿐이라 **플러그인 훅의 등록 여부를 볼 수 없다**(그쪽은
// 플러그인 매니페스트에 있다). 플러그인 없이 손으로만 등록해 쓰는 사용자에게 단정형으로 "지우세요"를
// 내면, 지운 뒤 포착이 조용히 멎고 [9]는 다시 `옛 그룹 없음`으로 초록이 된다 — 이 절이 없애려던
// 무성 실패의 거울상이다. 두 힌트의 문면이 갈리는 것은 근거가 갈리기 때문이지 누락이 아니다.
//
