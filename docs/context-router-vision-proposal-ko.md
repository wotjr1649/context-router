...
1. **기능 카탈로그(구현됨)**: `ctx_insight` 제외 10개 기능은 전부 구현하고 golden fixture로 검증한다 (HANDOFF 확정 유지). 구현 여부와 노출 여부는 별개다.
...
- *"10개 구현 완료를 출시 목표로 삼지 말라"* — HANDOFF 확정("합의된 전체 범위를 끝내기 전에는 태그하지 않는다")과 충돌하므로 불채택. 계약 10개 구현+fixture는 v0.0.1 태그 조건으로 유지한다.
