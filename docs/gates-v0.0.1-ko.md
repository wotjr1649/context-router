...
| 11 | **스키마 토큰 예산**: 기본 3종 도구 정의 tokenizer 실측, 상한 기록 | `internal/mcp/mcp_test.go` `TestSchemaTokenBudget`(**신규, 계획3 T10**) — 기본 프로필(ctr_search/ctr_fetch/ctr_transform) tools/list 결과 직렬화 **4359 bytes**(근사 토큰 ~1089, bytes/4 — Claude 정확 tokenizer 비공개라 근사치, 바이트 상한이 실질 게이트). 상한 `maxToolSchemaBytes` = 최초 실측×1.2 반올림 = **5231 bytes**로 고정. v0.1에서 6-도구 표면(세션 3종+scope 확장)으로 **12029 bytes** 재기준화 — `docs/gates-v0.1-ko.md` 참조 | PASS |
...
|---|---|---|---|---|---|---|
| Claude Code | windows | PASS (3종) | PASS | (3a) PASS(스크립트드, PR #5) / (3b) 시도 완료 — ESC 입력은 동작(턴/호출 중단 관측)하나 호스트의 취소 전달은 **프로세스 워처 실측으로 미도달 확정**(하단 경위) → 비차단(협의), 서버·호스트 비정상 종료 없음 | 2026-07-19 | ctr_search 정상(빈 스토어 hits:[]) + ctr_transform 오류 경로(256MB 메모리 킬) — 서버 생존·후속 호출 정상. 참고(3b 판정 아님): (3b) 준비 호출 2회는 ESC 미입력으로 10s 상한 도달 — 그때도 서버 생존·후속 정상 |
...
6라운드 + Codex 2패스 — 반영). **(3b) 결과(프로세스 워처 실측으로 확정)**: worker가 별도 프로세스(`ctr.exe
__transform-worker`)임을 이용해, 백그라운드 워처(200ms 폴링)로 worker 수명을 기록하는
판정법을 세웠다 — 취소가 서버에 도달하면 worker는 ESC 시점(수 초)에 죽고, 미도달이면
핸들러 10s 상한까지 산다. 실측 3종: ① 첫 시도(단일 호출, 워처 없음) — ESC가 호출을
클라이언트 레벨에서 즉시 중단시켰고(rejection 반환) 직후 `ctr_search` 정상(서버 생존);
② 대조군 2회(취소 없음) — worker 2개 동시 기동, 수명 9.3~9.7s(10s 상한 완주);
③ ESC 라운드(워처 가동) — 사용자 ESC가 눌렸으나(직후 턴 중단이 객관 증거) 실행 중이던
호출 2건은 취소되지 않고 자연 완주(10s 오류 정상 반환), worker 수명 9.7s = 대조군과 동일
→ **notifications/cancelled가 서버에 도달하지 않았음을 프로세스 수명으로 확정**. 결론:
호스트(Claude Code)의 ESC→취소 전달은 실측상 미발생/비결정적(session-03 추정의 실증,
호스트측 한계). 모든 라운드에서 호스트·서버 비정상 종료 없음 — 원 게이트 문언 충족.
서버측 취소 계약은 (3a)가 CI 상주로 보증하므로 사전 협의대로 비차단. 작성 과정의 실측 발견 2건:
