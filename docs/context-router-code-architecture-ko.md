...
> 준수: D8(프레임워크 금지 — 개수 상한이 아니다, 설계 §0 v0.17 개정) · D13(반파편화) · HANDOFF 최소화 원칙(ponytail full)
...
- **store ctx-aware Open 변형 1건(v0.2, 설계 §5 Shadow Recall)**: `OpenContext(ctx, dir, readOnly)` 신설 — writable 오픈의 open-lock 대기(`lockStore`)를 ctx로 관측해 deadline/취소 시 5s 하드 한도 이전에 `ErrUnavailable`로 포기한다. 훅은 deadline 예산(설계 §2.3) 안에서 content.db 잠금 경합에 걸려도 fail-open해야 하는데 기존 `Open`은 ctx가 없어 최대 5s를 무조건 대기했다. `Open(dir, readOnly)`은 `OpenContext(context.Background(), …)`로 위임해 기존 40여 호출부와 무기한 대기 동작을 그대로 유지한다 — `Open`에 ctx 시그니처를 강제 전파(전 호출부 파급)하는 대신 대기 변형 1건만 추가한 D13 예외(파편화 아닌 얇은 seam). 내부 `lockStoreCtx`가 background ctx일 때 기존 `lockStore`와 완전히 동형이라 기본 동작은 불변.
...
| `internal/cli/codex_toml_valid.go` 신설(15줄·함수 1개) vs 기존 파일 편입 | **D13 예외** | 파편화가 아닌 파서 접점 격리 — 이 패키지가 TOML 파서를 부르는 유일한 자리를 한 함수로 좁혀 D80 "파서 비의존"의 경계를 눈으로 확인한다(`internal/buildinfo` 예외 선례와 동형, 스펙 v0.17 D89·§1.3) |
