...
- **D78** `.ps1` 종료 신호를 `Register-EngineEvent`로 승격 + 최상단 전용 구문
  폴백: `snippetContent`(`internal/exec/exec.go`)가 만드는 스크립트를
  ...
  "'", "''")` — 현재 `nativeExitTail`이 하는 처리와 같다). 이 처리가 빠지면
  ...
  - **닫히는 공백 둘**(v0.13 §1.2에서 제거한다):
    ...
    ② "**종료 코드 0으로 끝나는 `exit`**은 네이티브 실패를 가린다" — 인수 없는
    `exit`과 `exit 0`이 같은 자리다. 보강의 트리거가 "인수 없는 `exit`"이 아니라
    아래 **보강 조건**의 `exit_code == 0`이기 때문이며, 구현 단계 실측에서
    `exit 0`도 동일하게 보강된다(exit=0 · stderr 빈 값 · 사이드파일 기록).
    `CHANGELOG.md`의 0.14.0 항목이 이 넓은 형태로 적혀 있다.
  - **보강 조건은 D76 그대로다.** `Run`의 보강 분기가 `.ps1` · `exit_code == 0` ·
  ...
  - 마커 판별(`ctrNativeExitMarker`)과 읽기 상한(`maxNativeExitSideFileBytes`)은 불변이다. nonce를 도입하지 않는다.
...
1. **D77 게이트** — `elapsed >= 1000ms`면 `Fatalf`. 다음을 함께 요구한다.
   ...
   `rep.Hashes`까지 넷이 맞아야 전 해시가 회수 경로를 밟았음이 함의된다. 다만
   **넷만으로 엄밀히 함의되지는 않는다** — `store.go:1241`의 `os.IsNotExist` 갈래는
   카운터를 하나도 올리지 않고 조용히 넘어간다. 이 게이트에서 함의가 성립하는 것은
   픽스처의 `ageBlobFile`이 블롭 파일 부재를 **셋업 실패**로 만들기 때문이다.
   ...
   캡처하고 원복한다.
2. **D78 종료 코드 동일성** — 승격 대상 네 케이스(`exit N`·정상·`throw`·**종료
   코드 0으로 끝나는 `exit`**)에서 승격 전후 종료 코드가 같다. 넷째의 표본은 인수
   없는 `exit` 하나로 두되 그 자리는 `exit 0`을 함께 뜻하며(§0 공백 ②), `exit 0`이
   같은 보강을 받는다는 사실은 아래 8번(신호 생성)이 사례로 감시한다.
...
9. **D78 한계 고정** — 경로마다 결과가 다르므로 따로 단정한다.
   ...
   → **스니펫이 정한 값이 사이드파일에 남는다**(`readNativeExitCode`).
   · **`-Force` 없는 `Unregister-Event`는 실패해 진짜 값이 남는다** — 이 대비가
   깨지면 `-SupportEvent`가 사라진 것이다.
10. **D79 축 판정** — 축마다 ① 전 패키지 빌드 ② 스코프 포함 ③ lint 결과 순으로
    확인한다. ①은 `GOOS=<축> go build ./...`이고, 실패는 "그 축이 성립하지
    않음"으로 읽는다 — lint 실패와 구분해 보고한다. **perf 축만 예외다** — `go build`는
    `_test.go`를 컴파일하지 않고 이 저장소에서 perf 태그가 거는 파일은
    `internal/search/perf_test.go` 하나뿐이라, `go build -tags perf ./...`는 태그
    없는 빌드와 **같은 파일 집합**을 컴파일해 이 축에서 **항상 통과한다**(구현
    단계 실측: `perf_test.go`를 일부러 컴파일 불가로 만들어도 종료 코드 0). 그래서
    perf 축의 ①은 빌드가 아니라 **테스트 컴파일**이다 —
    `go test -tags perf -run '^$' -p 1 ./...`(`.github/workflows/ci.yml:173`).
    ②의 perf 축은 `.TestGoFiles`/`.XTestGoFiles`를 본다.
...
- **`GOOS=darwin`·`GOOS=netbsd` 축의 1단계가 테스트 파일을 컴파일하지 않는다**
  ...
  모두의 보고 범위에 들어 있다. 그 파일이 이 축들에서 타입 체크에 실패하면 **1단계는
  초록인데 3단계가 `typechecking error`로 죽어**, 1단계가 존재하는 이유인 "축 실패
  ↔ lint 실패" 신호 분리가 무너진다. **이번에는 CI를 고치지 않는다** — 최종 리뷰가
  대안 셋을 모두 더 나쁜 것으로 판정했다. `go vet`은 golangci-lint 기본 린터에 이미
  들어 있는 `govet`과 출력이 섞여 바로 그 신호 분리를 없애고,
  `GOOS=darwin go test -run '^$' ./...`는 크로스 GOOS라 컴파일이 아니라 **실행
  단계**에서 `%1 is not a valid Win32 application`으로 죽으며(perf 축이 같은 관용구를
  쓸 수 있는 것은 그쪽이 네이티브 GOOS이기 때문이다), 패키지별 `go test -c`는 스텝을
  부풀린다. v0.15 후보로 남긴다 — `run_unix_test.go`가 실제로 그 축에서 깨지는 날이
  재검토 입력이다.
