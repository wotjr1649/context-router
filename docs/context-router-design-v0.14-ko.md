...
1. **D77 게이트** — `elapsed >= 1000ms`면 `Fatalf`. 다음을 함께 요구한다.
   ...
   `rep.DeferredFiles == 0`. 이 두 줄이 없으면 어떤 경로를 쟀는지 사후에 판별할
   수 없다.
...
`DeferredFiles=0`). 실 보유 시간 **19.8ms**(해시당 약 1.0ms)로 임계 1000ms 대비
여유 **50.5배**다. `-count 5` 재실행 대역은 13.0~23.7ms(여유 42.1~76.8배)이며,
최악 관측 23.7ms도 §2.1 규모 조건의 상한(임계의 1/5 = 200ms)의 8분의 1 아래다.
원 실측 632·633ms는 실 도그푸딩 저장소(아티팩트 1254개·210MiB)의 값이라 절대값
비교 대상이 아니다 — 이 게이트가 잡는 것은 해시당 비용의 회귀다.

**D77 유도 FAIL 2건**(감시선이 물린다는 증거). ① `ageBlobFile` 호출을 주석
처리하면 `회수 경로를 타지 않았다: ReclaimedB=0 DeferredFiles=20`. 나이 게이트가
단락되면 `stillReferenced`도 `os.Remove`도 실행되지 않아 유예 경로만 재는 종이
게이트가 된다는 §0 D77의 근거가 실측으로 확인된다. ② `budget`을 1ms로 낮추면
`잠금 보유가 예산을 넘었다: 18.56ms >= 1ms (hashes=20 reclaimed=370B deferred=0)`.
`Fatalf` 문면이 세 값을 함께 실어 "회귀인가 러너 편차인가"를 가를 수 있다는 것을
같은 출력이 함께 보인다. 둘 다 확인 후 원복해 PASS를 재확인했다.
