...
| `internal/cli/purgelog.go` **(신규)** | 파서 + 파일명 미러 상수 | ~80줄 |
...
- Produces: 후속 태스크 전부가 이것들을 쓴다 —
  ...
  - `store.PurgeLogName` 상수 = `"purge.log"`
...
// 상한, 빈 값은 "-". 그쪽 함수는 클로저이고 패키지도 달라 그대로 부를 수 없다 — 규칙이
// 갈리지 않는 것은 TestAppendPurgeLogSanitizes가 잠근다.
...
func writePurgeLog(t *testing.T, lines ...string) string {
	t.Helper()
	p := filepath.Join(t.TempDir(), purgeLogFileName)
...
Expected: 컴파일 실패 — `undefined: purgeLogTail`, `undefined: purgeLogFileName`
...
// purgeLogFileName — store.PurgeLogName 미러(dropsFileName 관례). 값이 갈리면
// TestPurgeLogFileNameMirrors가 잡는다.
const purgeLogFileName = "purge.log"
...
func purgeLogTail(path string, n int) (entries []purgeEntry, total int, unparsed int) {
	...
	for sc.Scan() {
		...
		if tsErr != nil || cErr != nil || fields[1] == "" {
			unparsed++
			continue
		}
...
- [ ] **Step 4: 미러 상수 테스트를 더한다**

`internal/cli/purgelog_test.go` 끝에:

```go
// TestPurgeLogFileNameMirrors — 미러 상수가 갈리면 doctor가 없는 파일을 읽는다.
// dropsFileName이 같은 이유로 같은 형태의 잠금을 갖는다.
func TestPurgeLogFileNameMirrors(t *testing.T) {
	if purgeLogFileName != store.PurgeLogName {
		t.Fatalf("미러 상수 %q != store.PurgeLogName %q", purgeLogFileName, store.PurgeLogName)
	}
}
```

`internal/cli/purgelog_test.go` 의 import 에 `"github.com/wotjr1649/context-router/internal/store"` 를 더한다.
...
먼저 **분류 우선순위 표 테스트**(스펙 §6 항목 6). 예산 소진·종료 취소를 e2e 로 유도하는 것은 비싸고 불안정하므로 분류를 순수 함수로 뽑아 표로 잰다 — 그리고 그 함수를 switch 가 부르게 해서 **로그와 기록이 같은 분류를 공유**하게 한다.

```go
// TestStartupPurgeStatusPriority — 스펙 §2.2의 우선순위가 계약이다. **partial이 budget·
// cancelled를 이긴다**: 행이 이미 삭제돼 다음 기동 술어에 다시 안 잡히므로, 중단 사유가
// 무엇이든 남은 파일의 유일한 회수 경로가 purge --gc다. 순서가 뒤집히면 "다음 기동이 다시
// 집는다"는 **거짓 안내**가 나간다. 이 표가 없으면 순서 교체가 조용히 통과한다.
func TestStartupPurgeStatusPriority(t *testing.T) {
	errX := errors.New("x")
	for _, tc := range []struct {
		...
		if got := startupPurgeStatus(tc.err, tc.hashes, tc.cancelled, tc.budget, tc.capped); got != tc.want {
...
그 다음 e2e. **Step 0 에서 찾은 헬퍼 이름으로 바꿔 쓴다.**
...
```go
		...
		purgeStatus := startupPurgeStatus(purgeErr, rep.Hashes, cancelled, budgetSpent, capped)
		switch {
...
같은 파일에 분류 함수를 더한다. **switch 에서 뽑아내는 것이 요점이다** — 표 테스트가 이 함수 위에 서고, slog 와 기록이 같은 답을 공유한다:

```go
// startupPurgeStatus — 기동 퍼지의 결과 분류(스펙 §2.2의 닫힌 집합). **우선순위가 계약이다**:
...
func startupPurgeStatus(purgeErr error, hashes int, cancelled, budgetSpent, capped bool) string {
	switch {
...
- `st.ProjectDir()` 가 없으면 **새로 만들지 말고** 그 고루틴이 이미 아는 프로젝트 디렉터리 경로 변수를 쓴다. 없다면 `store` 에 `func (s *Store) ProjectDir() string { return s.dir }` 한 줄을 더한다 — `s.dir` 이 이미 `lockStore(s.dir)` 에 쓰이는 그 값이다.
...
```bash
git add cmd/context-router/main.go cmd/context-router/main_test.go internal/store/store.go
...
func TestRunPurgeLogsOlderThan(t *testing.T) {
	...
	entries, total, unparsed := purgeLogTail(filepath.Join(projDir, purgeLogFileName), 5)
	...
	if !strings.HasSuffix(e.Policy, "/-") {
		t.Errorf("policy=%q — 사용자가 준 기간에는 출처가 없다(%q 접미)", e.Policy, "/-")
	}
}
...
func TestRunPurgeLogsGC(t *testing.T) {
	...
	entries, _, _ := purgeLogTail(filepath.Join(projDir, purgeLogFileName), 5)
...
- [ ] **Step 3: 세 호출 지점을 배선한다**

`runPurge` 의 `PurgeOlderThan` 직후 — 버려지던 둘째 반환값을 받는다:

```go
		_, artifacts, purgeErr := st.PurgeOlderThan(ctx, cutoffUnix)
		store.AppendPurgeLog(projDir, store.PurgeRecord{
			Path:   "cli-older-than",
			Policy: store.PurgePolicy(olderThan, ""), // 사용자가 준 기간에는 출처가 없다
			Status: purgeStatusOf(purgeErr),
			Cutoff: cutoffUnix,
			Count:  artifacts,
...
같은 함수의 `GCOrphanBlobs` 직후 — `removed` 를 받는다:
...
```go
		if purgeErr == nil && *gc {
			...
			store.AppendPurgeLog(projDir, store.PurgeRecord{
				Path: "cli-gc", Policy: "-", Status: purgeStatusOf(purgeErr), Count: removed,
			})
		}
```
...
`internal/cli/purgelog.go` 에 상태 헬퍼를 더한다 — **세 곳이 손으로 분기하면 갈린다**(D13):

```go
// purgeStatusOf — CLI 경로의 결과 분류. 기동 경로는 ctx 종료 사유로 여섯 갈래를 가르지만
// (cmd/context-router의 그 switch), CLI는 예산도 종료 취소도 없어 두 갈래뿐이다.
// **닫힌 집합을 벗어나지 않는다** — 스펙 §2.2의 값만 쓴다.
func purgeStatusOf(err error) string {
	if err != nil {
		return "failed"
	}
	return "ok"
...
**Interfaces:**
- Consumes: `purgeLogTail`, `purgeLogFileName`(Task 2)
...
func TestDoctorPurgeSectionAbsent(t *testing.T) {
	...
	// 부작용 축: "삭제 없음"·"0건"으로 읽히는 어휘가 그 줄에 없어야 한다.
	for _, banned := range []string{"삭제 없음", "0건", "없습니다"} {
		if strings.Contains(out, "[21] purges: "+banned) {
			t.Errorf("[21]이 %q로 읽힌다 — 부재와 0건은 다른 사실이다", banned)
		}
	}
}
...
func TestDoctorPurgeSectionLists(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(projDir, purgeLogFileName), []byte(seed), 0o600); err != nil {
		t.Fatal(err)
	}
...
```go
	...
	{
		entries, total, unparsed := purgeLogTail(filepath.Join(projDir, purgeLogFileName), 3)
		switch {
		case total == 0:
