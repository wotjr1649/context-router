...
**최종 게이트 리뷰 G1 정정(사후 반영)**: 이 Step이 원래 적었던 구현은 `os.ReadFile(path)`로 사이드
파일 전체를 마커 검사보다 먼저 서버 프로세스에 할당했다 — 스크래치가 스니펫의 작업 디렉터리이기도
해 스니펫이 그 경로에 임의 크기 파일을 직접 쓸 수 있으므로(F3 리뷰가 다루는 마커 위조와는 다른
축), 그 크기가 그대로 서버 쪽 할당이 된다. 위 코드는 `io.LimitReader(f, 상한+1)` +
`io.ReadAll`(`netfetch.readBody`·`hook.Run`과 같은 관용구)로 상한을 두고, 상한 초과·비일반 파일은
모두 "보강하지 않음"으로 처리한 최종 형태다. `io`를 새로 임포트해야 한다(`exec.go:9`) — `strconv`
(`exec.go:15`)·`log/slog`(`exec.go:10`)는 이미 있으므로 추가하지 않는다. 무조건 추가하면 중복
임포트로 빌드가 깨진다.
