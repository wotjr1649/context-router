...
T0(§3.1)는 헤드리스 신규 세션이라 `source=startup`에서 났다. `compact`에서 발화하는지, 그리고 그 주입이 압축 **후** 컨텍스트에 들어가는지는 별도 실측이다. 세션 60이 그것을 쟀으면 그 결과를 아래 형태로 적고, 안 쟀으면 `[미실측]`로 남긴 뒤 착수 조건에 올린다.

```markdown
**⑦ `compact` 실측.** <통과/실패 + 관측한 것 한 문장> `[실측 — <날짜>]`
```
