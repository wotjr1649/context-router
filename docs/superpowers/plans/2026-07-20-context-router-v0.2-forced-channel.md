...
- D13 배치: 신규 소스는 `internal/hook/hook.go`(+`hook_test.go`+`testdata/`) — 1,000줄 초과 시 `guard.go` 분리 1회만 허용. session 변경은 OpenAppend 계열만, ingest 변경은 `DeniedPath` 노출+`Request.SourceKind` 1필드만, cli는 hook install/uninstall·usage·doctor 항목 확장만.
...
- [ ] **Step 2: 실측 검증** — ⑤ 이 저장소에서 임시 PostToolUse 훅(stdin 바이트 수를 파일로 덤프하는 1줄 스크립트)을 `.claude/settings.local.json`에 등록, 대형 출력 도구 호출 1회로 `tool_response` 최대 크기·절단 정책 실측. 실측 후 임시 훅 제거.
- [ ] **Step 3: 픽스처 작성** — 검증된 실형태로 5개 JSON + README.md(발견 5건 + Shadow Recall 전제 성립 여부 판정). 절단이 확인되면 설계 §5의 "전문 확보" 전제 수정을 보고하고 중단(사용자 결정).
- [ ] **Step 4: Commit** — `"test(hook): 훅 stdin 골든 픽스처 + API 검증 기록 5건 (v0.2 설계 §1.3)"`
...
**Interfaces:**
- Consumes: `store.AcquireLock`(shared), 기존 `Append` 검증·상한 로직, `pragmas` DSN 상수.
- Produces:
  - `type AppendOptions struct{ ExternalSessionID, Producer string; RetentionSec int64 }` / `func OpenAppend(ctx context.Context, dir string, opts AppendOptions) (*AppendDB, error)` — 순서: ① shared lease(논블로킹, 실패 `ErrLeaseHeld`) ② recover 마커 → `ErrRecoverPending` ③ DB 미존재 시 init lock 생성 직렬화(Open과 동일 패턴) ④ PRAGMA+멱등 DDL — **단 busy_timeout은 500ms DSN 변형**(deadline 예산 안, 설계 §2.3) ⑤ **quick_check 생략**(손상은 append 시점 오류 분류로 사후 감지) ⑥ sessions INSERT·session_start **하지 않음**. `opts.ExternalSessionID`는 이미 `cc:` 접두사 완성형(형식 검증은 호출자 = hook, T4).
  - `func (d *AppendDB) EnsureSession(ctx context.Context, source, worktreeRoot string) (created bool, err error)` — `INSERT OR IGNORE INTO sessions(...)` + **신규 삽입 시에만** `session_start` append(payload: source·worktreeRoot·retention_sec).
...
**Files:** Create `internal/hook/hook.go`, `internal/hook/hook_test.go`; Modify `cmd/context-router/main.go`(서브커맨드 인지), `internal/cli/cli.go`(디스패치)
...
- Produces:
  - `func Run(ctx context.Context, stdin io.Reader, stdout io.Writer, storeRoot, version string, getenv func(string) string) int` — **항상 0 반환**. 순서: `CTR_HOOKS_OFF` → 즉시 0 / stdin JSON 파싱(hook_event_name·session_id·cwd·tool_name·tool_input·tool_response·source) → session_id canonical UUID 검증(불합격 = drop 기록 후 0) → `cc:<uuid>` 조립 → cwd로 ident 해석 → deadline ctx(`CTR_HOOK_DEADLINE_MS` 기본 2000) → 이벤트별 분기: SessionStart = EnsureSession / 그 외 = SessionExists false → drop(사유 `unknown-session`), true → 처리(T5~T7). `CTR_HOOK_RETENTION_SEC`(기본 2592000)을 읽어 `AppendOptions.RetentionSec`으로 전달(설계 §2.2 훅 세션 기본 retention 30일).
...
- Produces:
  - `func classify(in hookInput) (eventType, summary string, attrs map[string]any)` — 우선순위 error > git_diff/build_run/test_run(Bash 패턴표) > file_edit(Write/Edit/NotebookEdit) > tool_call. 패턴표는 패키지 var 정규식 슬라이스 3종(git: `^git (diff|commit|merge|rebase|log|status)`, build: `go build|dotnet build|npm run build|msbuild|make(\s|$)`, test: `go test|dotnet test|pytest|vitest|npm test`) — 테이블 테스트가 계약.
  - summary allowlist 조립: `<도구명>: <허용 요소>` — Bash 첫 토큰은 `^[A-Za-z0-9_./-]+$` 일치 시만 원문, 불일치(env 할당 `KEY=값` 등) → `<arg>`. 파일 도구는 워크스페이스 상대 경로. 오류는 정규화 분류·코드만. 조립 후 `ingest.Redact` 통과 + redaction 상태 기록. attrs도 allowlist 필드만(exit_code·bytes·matched_pattern·relative_path).
  - error 판정: tool_response의 오류 신호(T0 검증 형태) → event_type=error(분류 최우선).

- [ ] **Step 1: 실패 테스트** — ① 분류 테이블 테스트(git/build/test/기본 각 2케이스 + 우선순위: 오류 있는 test 명령 → error) ② file_edit(Write 픽스처 → 상대 경로 summary) ③ env 할당 마스킹(`SERVICE_KEY=abc deploy` → `<arg> deploy` 형태가 아니라 첫 토큰 `<arg>`) ④ secret canary(분할 리터럴)를 tool_input에 심고 → summary·attrs에 원문 부재 ⑤ 상한: summary 2048B 절단 ⑥ 픽스처 round-trip: posttooluse-bash.json → tool_call 1건 저장·FTS에서 canary 미회수(**§10 canary 게이트 세션 측**).
...
**Files:** Modify `internal/hook/hook.go`(+test), `internal/ingest/ingest.go`(`DeniedPath` 노출 + `Request.SourceKind`), `internal/ingest/ingest_test.go`

**Interfaces:**
- Consumes: `ingest.Run`(Content 경로), `store.Open`(content.db — deadline ctx 전파), T5 분류 결과.
- Produces:
  - ingest 예외 2건: `func DeniedPath(path string) bool`(기존 denylist 매처 공개 래퍼 — 로직 이동 없음), `Request.SourceKind string`(기본 "inline", 훅은 "hook" — SourceMeta.Kind로 전달).
  - hook 내 `shadowCapture(ctx, storeRoot ...)` — 판정 순서: `CTR_SHADOW_OFF` → skip / tool_response 직렬화 크기 ≤ `CTR_SHADOW_MIN`(16KiB) → skip / **> `CTR_SHADOW_MAX`(1MiB) → 미저장 + drops(`shadow-oversize`)** / 파일 유래 도구(Read 등)면 tool_input 경로를 `DeniedPath` 대조 → 일치 시 skip(drops `shadow-denylist`) / 바이너리 판정(NUL 바이트 sniff) → skip / 통과 시 `ingest.Run{Content, SourceKind:"hook"}` 저장 → `artifact_created` 이벤트 + 기본 이벤트에 더해 `tool_result_summary` 이벤트(artifact ref `artifact://<cc:...>/sha256-<hash>` — 문자열 조립만, url.Parse 금지) append. 여기까지 9종 중 8종(warning은 T7 guard가 생산).
- [ ] **Step 1: 실패 테스트** — ① 16KiB 이하 → 미저장 ② 초과 → content.db 아티팩트 + artifact_created + tool_result_summary(ref 형식 assert) ③ 1MiB 초과 → 미저장 + drops ④ Read 응답 + denylist 경로(`.env` 픽스처) → 미저장 + drops ⑤ NUL 포함 바이너리 → 미저장 ⑥ secret canary(분할 리터럴)가 응답 본문에 → 저장본에서 span redaction 적용(**§10 canary 게이트 shadow 측**) ⑦ SourceKind="hook"이 SourceMeta.Kind로 저장 ⑧ 해시 dedup(같은 응답 2회 → 아티팩트 1개).
- [ ] **Step 2: FAIL 확인** — `go test -p 1 ./internal/hook/ ./internal/ingest/ -run 'TestShadow|TestDeniedPath' -v`.
- [ ] **Step 3: 구현** — ingest diff 최소(공개 래퍼+필드 1개). shadow 판정은 hook 소유.
...
**Files:** Modify `internal/cli/cli.go`(+test)

**Interfaces:**
- Consumes: T0 settings 스키마 검증 결과, T4 cli 디스패치.
- Produces:
  - `context-router hook install [--user] [--no-shadow] [--store-root <p>]` — 대상: 프로젝트 `.claude/settings.json`(기본) / `--user` = `~/.claude/settings.json`. **병합 계약**: 기존 JSON 파싱 → hooks의 해당 이벤트 배열에서 자기 항목(명령이 `context-router hook`로 시작) 제거 후 append(버전 마커 필드) → **temp 파일+rename 원자 쓰기** → 미지 키·타 도구 훅 항목 왕복 보존. 등록: SessionStart / PreToolUse(matcher "Read") / PostToolUse(matcher 전체) + timeout 10. `--no-shadow` = 훅 명령에 `CTR_SHADOW_OFF` 반영(env 또는 args — T0 스키마 검증 결과에 따름), `--store-root` = args에 명시 주입. `uninstall`이 자기 항목만 대칭 제거.
...
- [ ] **Step 1: 실패 테스트** — ① 빈 설정에 install → 3항목 등록·유효 JSON ② 재install 멱등(항목 1벌 유지) ③ **타 도구 훅 항목+미지 키 시드 후 install/uninstall → 원형 보존** ④ uninstall 대칭(자기 항목만 제거) ⑤ 원자성(임시 파일 잔존물 부재) ⑥ doctor: 훅 등록/미등록 각 상태 문구 + drops 건수 표기 + 항목 순서 [1]~[n] 오름차순 ⑦ `--no-shadow`/`--store-root` 반영.
...
**Files:** Modify `internal/cli/cli.go`(+test), `cmd/context-router/main.go`(usage 서브커맨드 인지)

**Interfaces:**
- Consumes: `session.OpenReadOnly`(cc: 세션 존재 조회), Claude Code transcript JSONL(`~/.claude/projects/<경로 문자 치환 디렉터리>/*.jsonl`).
...
- [ ] **Step 1: 실측 검증** — 이 머신의 `~/.claude/projects/` 실디렉터리명과 transcript JSONL 1개의 usage 필드 형태(필드명·중첩)를 확인해 파서 계약 고정(설계 §6 [plan 검증]). 결과를 테스트 픽스처(축약 JSONL, testdata)로 고정.
...
**Files:** Create `internal/hook/e2e_test.go`(또는 기존 E2E 파일 관례 위치 — v0.1 T10 패턴 승계)

**Interfaces:** Consumes 전 태스크. Produces 게이트 실체:
- E2E: 테스트가 바이너리 빌드 후 실프로세스로 `context-router hook`을 픽스처 stdin과 실행(SessionStart→PostToolUse 3건) → session.db에 `cc:` 세션·이벤트·아티팩트 검증. MCP 서버 프로세스(기존 E2E 하네스) 동시 가동 상태에서 훅 append 무손실(content.db·session.db 동시 쓰기).
- deadline 결정론: 테스트가 exclusive lease 선점 후 훅 실행 → `CTR_HOOK_DEADLINE_MS=300` 안에 종료(측정)·exit 0·drops 1줄.
...
- [ ] **Step 1:** 이 저장소에서 `context-router hook install` 후 실세션 1회 — `session_start`/`tool_call`/`file_edit`/`test_run` 관측, doctor GREEN(훅 항목 포함).
- [ ] **Step 2:** `--profile ingest`로 fetch 정상 경로 스모크 1회(ctr_index → search → fetch byte-exact, session-08 §4.2 이월).
