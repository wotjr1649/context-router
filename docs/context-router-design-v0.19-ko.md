...
★ **출력을 *대체*하는 경로는 없지만 *덧붙이는* 경로는 있다** `[실측 — 세션 60, 2026-08-12]`.
`PostToolUse`의 `hookSpecificOutput.additionalContext`는 모델 컨텍스트에 **실제로 닿는다**:
위와 같은 형태(고정 마커만 내는 프로젝트 스코프 훅을 `Glob`에 건다)로 불렀더니 그 도구 결과
뒤에 `PostToolUse:Glob hook additional context: <마커>`가 그대로 왔고, sentinel이 발화도 함께
확인했다. **이것이 위 기각을 뒤집지는 않는다** — 대체가 아니라 추가라 컨텍스트 절약은 0이다.
바뀌는 것은 다른 축이다: **포착이 일어났다는 사실과 그 artifact 참조를 모델이 알게 된다.**
D104가 재려는 회수는 그 앎 없이 시작되지 않으므로, 이 필드는 (b)가 아니라 **(a) 가시성**의
수단이다. 문면은 계약 2를 그대로 받는다 — 대체 목적지만 말한다.

★ **Windows에서 훅 `command`의 경로는 슬래시로 적는다** `[실측 — 같은 프로브]`. 같은 훅을
백슬래시 절대경로로 걸었을 때는 마커도 sentinel도 나오지 않았고, 슬래시로 바꾸자 둘 다 나왔다.
훅 미발화인지 스크립트 미실행인지를 이 프로브는 가르지 못하지만 **어느 쪽이든 백슬래시 경로가
훅을 조용히 무력화한다.** 우리가 배송하는 훅은 PATH 기반(`context-router hook …`)이라 이
자리에 걸리지 않는다.
