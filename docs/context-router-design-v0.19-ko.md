...
4. **수동 포착을 1급 후보로 둔다.** PostToolUse가 큰 결과를 스스로 색인하면 채택률과 무관하게
   값이 난다. **단, Bash 출력은 담지 않는다** — 참조 구현이 그 이유를 적는다: 큰 Bash 출력은
   자격증명을 흔히 담고, 수동 저장은 일시적 노출을 **영속·프로젝트 전역 검색 가능**으로
   바꾼다. 이것은 S4 경계이고 설계 결정으로 못박는다.
...
우리가 파일을 쓰는 걸음이 없다. 프로필과 `--store-root`는 매니페스트 `userConfig` 스키마 +
`--config <key=value>`로 옮길 수 있다 `[실측: 두 호스트 다 그 플래그가 있다]`
`[미실측: 스키마의 실제 형태]`.
...
1. **Codex에서 훅이 발화하는가** `[미실측 — 시도했으나 못 쟀다]`.
   **Claude 쪽은 닫혔다** `[실측]`: 플러그인이 선언한 `PreToolUse` 훅이 이 Windows에서 발화하고
   (마커 파일이 기록됨) exit 2가 거부로 존중된다(읽기가 실제로 차단됨).
   **Codex 쪽은 격리 홈에 인증이 없어** 모델 연결이 401로 끊기고 세션이 훅 디스패치 지점에 닿지
   않는다. `SessionStart`에 `matcher` 보강 · `commandWindows` 추가 · `--enable hooks` · 훅 신뢰
   우회까지 넣어도 마커가 안 찍혔고 `[hooks.state]`도 생기지 않았다 — **401이 교란 변수라
   "발화하지 않는다"로 읽어서는 안 된다.**
   간접 증거는 강하다: 진짜 홈의 `config.toml`에 설치된 플러그인 셋에 대해
   ``[hooks.state."<plugin>@<mp>:hooks/<파일>.json:<이벤트>:i:j"] trusted_hash = "sha256:…"``가
   있다 `[실측]` — Codex가 플러그인 훅 파일을 열어 항목을 열거하고 항목 단위로 해시했다는 뜻이다.
   **인증 있는 Codex 세션에서 한 번 재면 닫힌다.**
   *미해결 관측 하나*: `cmd /c exit 2`만 둔 훅은 차단하지 못했고, 같은 exit 코드에 앞선 명령을
   더한 형태는 차단했다. 원인 불명 — 거부를 설계에 쓰기 전에 확인하라.
2. **훅 파일 하나로 양쪽을 섬길 수 있는가** `[미실측]`. 실제로 도는 Codex 플러그인 훅 파일은
   `SessionStart`에도 `matcher`를 달고 Windows용 **`commandWindows`**를 함께 담는다
   `[실측: 설치된 플러그인 둘의 파일]`. 합치려면 그 둘을 갖춰야 하고, 갖추고도 되는지는 1번과
   함께 재라.
3. **`outputSchema`가 모델 컨텍스트에 실리는가** — **닫혔다. 실리지 않는다**(B①). D99 계약 4가
   그 결론이다.
4. **진입 도구 집합** `[미확정]` — D99 계약 1. **표현 수단은 있다** `[실측: go doc]`: 설치된
   SDK의 `mcp.Tool`이 ``Meta `json:"_meta,omitempty"` ``를 담는다. 참조 구현이 쓰는 키는
   `"anthropic/alwaysLoad"`다.
5. **`userConfig` 스키마의 실제 형태** `[미실측]`.
6. **마켓플레이스 배포** `[미실측]` — 저장소 자신이 마켓플레이스인 형태(`source: "./"`)가
   원격 git 소스로도 양쪽에서 되는지. 로컬 경로로는 됐다.
7. **버전 번호 판정.** 도구 이름공간 이동은 사용자의 권한 규칙을 깬다. `0.19`인지 `1.0`인지는
   소유자 판정이며 이 문서는 파일명만 델타 체인에 맞췄다.
8. **수동 포착(D100 계약 4)의 도입 여부** — 값과 위험을 우리 숫자로 재기 전에는 후보다.
9. **`[features] hooks`** — 이 호스트의 `config.toml`에 그 키가 있고 `codex features list`가
   `hooks`를 `stable true`로 낸다 `[실측]`. 참조 구현 README가 요구하는 `plugin_hooks`는
   `removed` 상태다 `[실측]`. **새 환경의 기본값은 미확정.**
