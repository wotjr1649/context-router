...
**근거**: B①이 지연의 이득을(+399 대 +3,235), B②가 지연의 대가를(62일 0회) 잰다. 둘 다 참이므로
전부-상시도 전부-지연도 틀렸다. 참조 구현이 같은 자리에서 11개 중 2개만 상시로 두고 나머지
아홉을 설명 꼬리에 색인하는 형태로 수렴했고, 그 근거를 이렇게 적는다 — *"their descriptions
...
4. **`outputSchema`를 재검토한다** — `[실측]` 도구 배열의 **약 51%**가 `outputSchema`다
   (`ingest,net`에서 10,914자 중 5,594자). 호스트가 그것을 모델에 싣는지는 `[미실측]`.
...
1. **훅이 실제로 발화하는가** `[미실측]`. 매니페스트 적법성·인벤토리·MCP 마운트까지만 쟀다.
   훅 `command`는 셸 문자열 경로라 A⑤의 반례가 유일하게 남아 있을 수 있는 자리다. 양쪽에서 재라.
2. **훅 파일 하나로 양쪽을 섬길 수 있는가** `[미실측]`. Codex의 `is_exact_matcher` 아래에서
   `mcp__.*`가 Claude에서도 같은 집합을 잡는지.
3. **`outputSchema`가 모델 컨텍스트에 실리는가** `[미실측]`. D99 계약 4의 전제다. 실으면
   페이로드의 절반이 거기 있고, 안 실으면 D99의 절감 추정이 절반으로 줄어든다.
4. **진입 도구 집합** `[미확정]` — D99 계약 1. 그리고 도구 단위 상시 로드를 우리 Go SDK가
   낼 수 있는가(`_meta` 필드).
