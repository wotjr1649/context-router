...
- **훅 그룹 소유 판정은 전건이다 — 혼합 그룹은 불가침이고, 그 대가를 우리가 진다.**
  ...
  **그 대가는 진단으로 번진다.** `scanRegisteredHooks`가 같은 술어를 쓰므로 혼합 그룹만 남은
  사용자에게 `doctor [9]`·`[16]`은 `옛 그룹 없음`을 낸다 — **우리 명령이 파일에 남아 계속
  발화하는데 진단이 잔존을 짚지 않는다.** 이것은 v0.18 D92 계약 7이 **"틀린 성공"**이라 부른
  형태의 사촌이다: 그쪽은 지우지 못한 것을 "제거 완료"라 말했고, 이쪽은 남긴 것을 "없음"이라
  말한다. 두 자리의 차이는, 여기서는 **남기는 것이 의도된 판정**이고 정리 경로가 사용자에게
  있다는 것이다(호스트의 `/hooks`에서 그 항목을 지운다). 그래서 조치는 "보류 문면"이 아니라
  **문서화**다 — CHANGELOG의 `hook uninstall` 항목이 이 형태를 명시하고,
  `TestDoctor_MixedGroupReportsNoLegacyGroups`가 그 조합을 고정한다. **더 나은 자리**:
  `[9]`·`[16]`이 혼합 그룹을 별도 부류로 세어 "사용자 항목과 동거 중인 우리 항목 N개"를
  짚는 것. 소유 판정을 바꾸지 않고 보고만 늘리는 변경이라 이번 릴리스 밖에 둔다 `[미실측]`.
