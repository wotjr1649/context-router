...
- **D44** Codex usage 어댑터: `~/.codex/sessions/YYYY/MM/DD/
  ...
  **v0.4 기각(§1.2 "세 번째 저장 형식 계약"·사용자 결정 ①)의 명시적
  ...
  `session_meta.session_id`(파일명은 발견 수단), **파일** 토큰 총계는
  **max-wins — 파일 내 token_count 중 `total` 최대 스냅샷의 벡터 전체
  채택**, **세션은 같은 session_id의 파일 그룹 합산**(재개·포크가
  세션당 복수 파일 생성·파일 간 누적 독립 재시작 — 도그푸딩 실측으로
  "파일=세션" 폐기, §2·§9).
...
- 파일당 파싱: `session_meta`의 `session_id`가 세션 권위 키(재개 세션은
  ...
  수단이며 불일치 시 meta 우선+집계). **파일 토큰 총계는 max-wins**:
  파일 내 전체 `token_count` 중 `total`이 최대인 스냅샷의
  `total_token_usage` 벡터 전체를 채택 — 스냅샷 단위 채택이라 필드 간
  정합이 보존되고, 리셋 판정·구간 분리가 불필요하다(근거: 재개 98건
  전수 단조·구간 내 감소 0 실측 §7; 감소 스냅샷은 재전송·순서 역전으로
  간주하고 무시). `input_tokens`는 `cached_input_tokens`를 포함하는
  상위 값(실측 §7) — 열 정의에 반영.
- **세션=파일 그룹(도그푸딩 발견 §9 — "파일=세션" 폐기)**: 재개·포크는
  같은 `session_id`로 **새 rollout 파일**을 만든다(실측 §7: 프로젝트
  귀속 50파일=21세션, 최대 13파일/세션 — 파일명 UUID 중복 0은 파일명
  축의 사실일 뿐 세션 축이 아니다). 파일 간 누적은 **독립 재시작**
  (실측 §7: 경계 23/25 RESTART, 예외 2건은 단일 이벤트 소액 —
  독립 스트림 합산이 정확·안전). 따라서 **세션 총계 = 소속 파일별
  max-wins 벡터의 합**, turns = 파일 합, 세션 시작 = 최초 파일 ts,
  arm 판정·창 게이트·커버리지는 세션 단위. 같은 세션의 파일을 하나만
  남기는 dedup은 체계적 과소계상이라 금지.
...
- 전체 회귀 `go test -p 1`(메모리 캡 규율). 도그푸딩 수용 기준:
  ...
  캐비앗 출력, cx: 토큰 총계가 §7 기준선(루트 cwd 50파일=21세션 합계 —
...
- rollout 실측(3차 — 도그푸딩 수용 중, 세션=파일 그룹 확정): 루트 cwd
  50파일의 meta `session_id`는 **21개 유일값**(019f5fb8=13파일·
  019f75af=10파일·019f7aff=6파일·019f5f7f=4파일 등) — 재개·포크가 같은
  세션 id로 새 파일을 만든다("파일=세션" 반증). 같은 세션의 인접 파일
  경계 25건 중 **23건 RESTART**(다음 파일 첫 total이 ~25–27K 기본
  프롬프트 수준으로 복귀), 예외 2건은 단일 token_count 소액 파일 —
  파일별 독립 누적 스트림이며 세션 총계=파일별 max의 합이 정확.
  2차 기준선의 50파일 합산 수치는 그대로 유효(독립 스트림 합)이고
  "50파일=50세션" 해석만 폐기.
...
### 도그푸딩 수용 발견 (구현 후 — §6 수용 기준 작동)

- 실데이터 `usage --compare` 대조에서 채택 세션 21 vs 기준선 50파일
  불일치 → 추적 결과 **"파일=세션" 전제 반증**(§7 3차): 재개·포크가
  같은 `session_id`로 복수 파일 생성, 파일 간 누적은 독립 재시작.
  초판 계약("같은 session_id 재등장은 첫 파일만 — 방어적 dedup")은
  체계적 과소계상(최대 13파일 세션이 1파일로 계상)이라 **세션=파일
  그룹, 세션 총계=파일별 max-wins 합**으로 D44·§2 교정. 파일 내
  max-wins 계약(2패스 확정)은 불변 — 합산 단위만 파일→세션 그룹.
  수용 기준(§6 기준선 재계산 일치)이 설계한 대로 결함을 포착한 사례.
