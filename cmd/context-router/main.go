...
// `LIMIT`은 반환 행만 묶고 shadow 술어 평가는 묶지 않는다. v0.13 D73이 이 술어가 쓰는
// `sources.raw_blob_hash`·`artifact_id` 조회에 복합 색인 3개(shadowIndexDDL)를 추가했지만,
// 실측(설계 v0.13 §3, D73 §2⑧)은 색인이 이 술어를 항상 빠르게 하지는 않음을 보였다 — sources가
// artifact_id 하나에 몰리는 합성 최악 사례에서는 색인 있는 쪽이 오히려 더 느렸다(2천행 ×1.78,
// 2만행 ×1.27, 두 경우 다 행 수 10배에 무색인 ×119.7·색인 ×85.6로 자람). 그래서 아래 보유 시간
// (632·633ms 등)은 **그 측정 당시(색인 없음, 아티팩트 1254개·210MiB)의 값 그대로**이며 D73 색인
// 도입만으로 낮아졌다고 볼 근거는 없다 — 실 저장소의 artifact_id 분포(다수 아티팩트에 고르게
// 갈리는지)에서 색인이 얼마나 도움이 되는지는 별도 실측이 필요하고 아직 없다. 배치 크기를
// 고정해도 shadow 아티팩트가 늘면 보유 시간이 함께 자란다는 결론 자체는 색인 유무와 무관하게
// 유효하다고 봐야 한다.
