...
// 그래서 위 보유 시간(632·633ms 등)은 **그 측정 당시(색인 없음, 아티팩트 1254개·210MiB)의 값
// 그대로**이고, D73 색인 도입 이후 실제 보유 시간을 다시 재지는 않았다 — §3의 실측은
// 술어(SELECT)만 격리해 쟀고, 행 삭제·파일 회수를 포함한 전체 PurgeHookOnlyOlderThan 보유
// 시간은 이 결정의 실측 대상이 아니다. 배치 크기를 고정해도 shadow 아티팩트가 늘면 술어 비용이
// 함께 자란다는 결론 자체는 색인 유무와 무관하게(정도만 다르게) 유효하다.
